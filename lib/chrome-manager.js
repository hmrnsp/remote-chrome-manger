...
// Resident memory for a whole set of instances using a single `ps` + single
// `lsof`, both async. Returns Map<instanceId, bytes|null>.
async function getInstancesMemoryBytes(instanceList) {
  ...
  const [snapshot, byPort] = await Promise.all([psSnapshot(), listeningPidsByPort()]);
  for (const instance of local) {
    const pids = byPort.get(instance.port);
    result.set(instance.id, pids && pids.size ? sumTreeRssBytes(pids, snapshot) : null);
  }
  return result;
}
